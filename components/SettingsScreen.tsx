  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-8 space-y-6 w-full max-w-md animate-slide-in-fade-in">
      <header className="text-center relative">
        <button
          onClick={toggleTheme}
          className="absolute top-0 left-0 text-slate-500 dark:text-slate-400 hover:text-[var(--color-primary)] dark:hover:text-yellow-400 p-2 rounded-full transition-colors"
        ...
        <h1 className="text-2xl font-bold text-[var(--color-header-light)] dark:text-[var(--color-header-dark)]">{t('welcome_message')}</h1>
              ...
              <div key={tree.id} className="relative group">
                <button
                  ...
                  className={`w-full aspect-square rounded-lg flex items-center justify-center p-1 transition-all duration-200 ${
                    isSelected ? 'bg-[var(--color-primary)]/20 dark:bg-[var(--color-primary)]/20 ring-4 ring-[var(--color-primary)]' : 'bg-slate-100 dark:bg-slate-700'
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center">
            ...
            <div className="flex items-center p-1 bg-slate-200 dark:bg-slate-700 rounded-full">
                <button onClick={() => setDurationMode('preset')} className={`px-3 py-1 text-sm font-semibold rounded-full transition-all ${durationMode === 'preset' ? 'bg-white dark:bg-slate-600 text-[var(--color-primary-hover)] dark:text-[var(--color-header-dark)] shadow' : 'text-slate-500 dark:text-slate-400'}`}>
                    {t('preset_label')}
                </button>
                <button onClick={() => setDurationMode('slider')} className={`px-3 py-1 text-sm font-semibold rounded-full transition-all ${durationMode === 'slider' ? 'bg-white dark:bg-slate-600 text-[var(--color-primary-hover)] dark:text-[var(--color-header-dark)] shadow' : 'text-slate-500 dark:text-slate-400'}`}>
                    {t('slider_label')}
                </button>
            </div>
        </div>
        ...
        {durationMode === 'preset' ? (
            ...
            <div className="space-y-2 pt-2">
                <input
                    ...
                    className="w-full h-2 bg-slate-200 dark:bg-slate-600 rounded-lg appearance-none cursor-pointer accent-[var(--color-primary)]"
                />
                <p className="text-center text-lg font-semibold text-[var(--color-primary-hover)] dark:text-[var(--color-header-dark)]">
      ...
      <div className="pt-4 space-y-3">
        <button
          ...
          className="w-full bg-[var(--color-primary)] text-white font-bold py-3 px-6 rounded-full hover:bg-[var(--color-primary-hover)] focus:outline-none focus:ring-4 focus:ring-[var(--color-primary-focus)] transform hover:scale-105 active:scale-100 transition-all duration-200"
        >
          {t('start_button')}
        </button>
        <div className="flex gap-3">
           <button
            ...
            className="w-full bg-[var(--color-tertiary)] text-white font-bold py-3 px-6 rounded-full hover:bg-[var(--color-tertiary-hover)] focus:outline-none focus:ring-4 focus:ring-[var(--color-tertiary)]/50 transition-transform duration-200"
          >
            {t('my_profile_button')}
          </button>
          <button
